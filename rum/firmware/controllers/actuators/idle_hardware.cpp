...
/**
 * Set IACV target in percent (0..100).
 * This wrapper menggunakan applyIACposition() yang sudah ada: ia akan memilih
 * stepper atau solenoid sesuai konfigurasi engineConfiguration->useStepperIdle.
 */
void iacvSetTargetPercent(float percent) {
    // clamp 0..100
    if (percent < 0.0f) percent = 0.0f;
    if (percent > 100.0f) percent = 100.0f;

    // applyIACposition accepts percent_t (scaled int), but accepts float in implementation
    applyIACposition((percent_t)percent);
}

/**
 * Return current IACV target in percent (0..100).
 * If stepper idle is used, read target steps from iacMotor and convert to percent.
 * Otherwise, return getIdlePosition() which gives current idle position percent.
 */
float iacvGetTargetPercent() {
    // If stepper is used, try to read the stepper target
#if !EFI_UNIT_TEST
    if (engineConfiguration->useStepperIdle) {
        extern StepperMotor iacMotor; // defined in idle_hardware.cpp file's scope
        // getTargetPosition() returns steps; convert to percent if totalSteps known
        const int totalSteps = engineConfiguration->idleStepperTotalSteps;
        if (totalSteps > 0) {
            float steps = iacMotor.getTargetPosition();
            float pct = (steps / (float) totalSteps) * 100.0f;
...
#endif
    // fallback: return getIdlePosition() (percent_t)
    percent_t pos = getIdlePosition();
    return (float)pos;
}
